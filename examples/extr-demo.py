...
def demo_visualization_only():
    ...
    # 获取输出目录
    data_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    calibration_dir = os.path.join(data_dir, "outputs", "example", "extrinsics")
    ...
    visualizer_poses.plot_poses_3d(
        ...
        save_path='outputs/example/visualization/poses_3d.png',
        show=False
    )
    ...
    visualizer_errors.plot_error_histogram(
        errors,
        save_path='outputs/example/visualization/error_histogram.png',
        show=False
    )
    ...
    visualizer_pc.plot_before_after_registration(
        ...
        transformed,
        points2,
        save_path='outputs/example/visualization/registration.png',
        show=False
    )

    print("\n可视化示例已保存到 outputs/example/visualization/ 目录")
